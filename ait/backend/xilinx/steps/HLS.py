...
def update_resource_utilization(acc):
    ...
    global project_acc_HLS_path

    report_file = project_acc_HLS_path + '/solution1/syn/report/' + acc.name + '_wrapper_csynth.xml'
...
def synthesize_accelerator(acc):
    ...
    project_acc_HLS_path = project_backend_path + '/HLS/' + acc.name

    shutil.rmtree(project_acc_HLS_path, ignore_errors=True)
    os.makedirs(project_acc_HLS_path)

    shutil.copy2(acc.full_path, project_acc_HLS_path + '/' + acc.filename)
    ...
    acc_tcl_script = '# Script automatically generated by the Accelerator Integration Tool. Edit at your own risk.\n' \
                     ...
                     + 'add_files ' + acc.name + '/' + acc.filename + ' -cflags "-std=c++11 -I' + os.getcwd() + '"\n' \
    ...
    acc_tcl_script_file = open(project_acc_HLS_path + '/HLS_' + acc.name + '.tcl', 'w')
    acc_tcl_script_file.write(acc_tcl_script)
    acc_tcl_script_file.close()

    msg.info('Synthesizing \'' + acc.name + '\'')

    p = subprocess.Popen('vitis_hls ' + project_acc_HLS_path + '/HLS_' + acc.name + '.tcl'
                         + ' -l ' + project_acc_HLS_path + '/HLS_' + acc.name + '.log',
