...
resize_pblock [get_pblocks eth_subsys] -add {SLR2}

add_cells_to_pblock [get_pblocks slr1_pblock] [get_cells -hierarchical -filter "NAME=~*eth_decoder_rs*slr_master*"]
add_cells_to_pblock [get_pblocks slr0_pblock] [get_cells -hierarchical -filter "NAME=~*eth_decoder_rs*slr_slave*"]
...
add_cells_to_pblock [get_pblocks slr0_pblock] [get_cells -hierarchical -filter "NAME=~*eth_encoder_rs*slr_master*"]
add_cells_to_pblock [get_pblocks slr1_pblock] [get_cells -hierarchical -filter "NAME=~*eth_encoder_rs*slr_slave*"]
