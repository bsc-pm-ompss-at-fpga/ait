...
namespace eval hwruntime {
    proc create_hwr_inStream_hier {} {
        ...
        for {set i 0} {${i} < [dict get ${AIT::project::aitConfig} "num_instances"]} {incr i} {
        ...
        if {[dict get ${AIT::project::aitConfig} "lock_hwruntime"]} {
    ...
    variable hier [create_bd_cell -type hier Hardware_Runtime]
    ...
    if {![dict get ${AIT::project::aitConfig} "disable_spawn_queues"]} {
    ...
    if {[dict get ${AIT::project::aitConfig} "enable_pom_axilite"]} {
    ...
    variable hwruntimeIP [create_bd_cell -type ip -vlnv bsc:ompss:picos_ompss_manager Picos_OmpSs_Manager]
    set POM_Config [list \
        CONFIG.AXILITE_INTF [dict get ${AIT::project::aitConfig} "enable_pom_axilite"] \
        CONFIG.CMDIN_SUBQUEUE_LEN [dict get ${AIT::project::aitConfig} "cmdin_subqueue_len"] \
        CONFIG.CMDOUT_SUBQUEUE_LEN [dict get ${AIT::project::aitConfig} "cmdout_subqueue_len"] \
        CONFIG.ENABLE_SPAWN_QUEUES [expr {![dict get ${AIT::project::aitConfig} "disable_spawn_queues"]}] \
        CONFIG.ENABLE_TASK_CREATION [dict get ${AIT::project::aitConfig} "task_creation"] \
        CONFIG.LOCK_SUPPORT [dict get ${AIT::project::aitConfig} "lock_hwruntime"] \
        CONFIG.MAX_ACCS [expr {max([dict get ${AIT::project::aitConfig} "num_instances"], 2)}] \
        CONFIG.MAX_ACC_CREATORS [expr {max([dict get ${AIT::project::aitConfig} "num_acc_creators"], 2)}] \
        CONFIG.MAX_ACC_TYPES [expr {max([dict size [dict get ${AIT::project::accs}]] + ([dict get ${AIT::project::aitConfig} "ompif"] ? 2 : 0), 2)}] \
        CONFIG.MAX_ARGS_PER_TASK [dict get ${AIT::project::aitConfig} "max_args_per_task"] \
        CONFIG.MAX_COPS_PER_TASK [dict get ${AIT::project::aitConfig} "max_copies_per_task"] \
        CONFIG.MAX_DEPS_PER_TASK [dict get ${AIT::project::aitConfig} "max_deps_per_task"] \
    ]
    ...
    if {![dict get ${AIT::project::aitConfig} "disable_spawn_queues"]} {
        lappend POM_Config \
            CONFIG.SPAWNIN_QUEUE_LEN [dict get ${AIT::project::aitConfig} "spawnin_queue_len"] \
            CONFIG.SPAWNOUT_QUEUE_LEN [dict get ${AIT::project::aitConfig} "spawnout_queue_len"] \
        }

    if {[dict get ${AIT::project::aitConfig} "task_creation"] && [dict get ${AIT::project::aitConfig} "deps_hwruntime"]} {
        lappend POM_Config \
            CONFIG.DM_DS [dict get ${AIT::project::aitConfig} "picos_dm_ds"] \
            CONFIG.DM_HASH [dict get ${AIT::project::aitConfig} "picos_dm_hash"] \
            CONFIG.DM_SIZE [dict get ${AIT::project::aitConfig} "picos_dm_size"] \
            CONFIG.ENABLE_DEPS [dict get ${AIT::project::aitConfig} "deps_hwruntime"] \
            CONFIG.HASH_T_SIZE [dict get ${AIT::project::aitConfig} "picos_hash_t_size"] \
            CONFIG.NUM_DCTS [dict get ${AIT::project::aitConfig} "picos_num_dcts"] \
            CONFIG.TM_SIZE [dict get ${AIT::project::aitConfig} "picos_tm_size"] \
            CONFIG.VM_SIZE [dict get ${AIT::project::aitConfig} "picos_vm_size"] \
        }

    set_property -dict ${POM_Config} ${hwruntimeIP}
    ...
    set_property -dict [list \
        ...
        CONFIG.Write_Depth_A [expr {[dict get ${AIT::project::aitConfig} "cmdin_subqueue_len"]*max([dict get ${AIT::project::aitConfig} "num_instances"], 2)}] \
    ...
    set_property -dict [list \
        ...
        CONFIG.Use_RSTA_Pin {false} \
        CONFIG.Use_RSTB_Pin {true} \
        CONFIG.Write_Depth_A [expr {[dict get ${AIT::project::aitConfig} "cmdout_subqueue_len"]*max([dict get ${AIT::project::aitConfig} "num_instances"], 2)}] \
        CONFIG.Write_Width_A {64} \
        CONFIG.Write_Width_B {32} \
        CONFIG.use_bram_block {Stand_Alone} \
    ] ${cmdOutQueue}
    ...
    if  {![dict get ${AIT::project::aitConfig} "disable_spawn_queues"]} {
        ...
        set_property -dict [list \
            ...
            CONFIG.Use_RSTA_Pin {false} \
            CONFIG.Use_RSTB_Pin {true} \
            CONFIG.Write_Depth_A [dict get ${AIT::project::aitConfig} "spawnin_queue_len"] \
            CONFIG.Write_Width_A {64} \
            CONFIG.Write_Width_B {32} \
            CONFIG.use_bram_block {Stand_Alone} \
        ] ${spawnInQueue}
        ...
        set_property -dict [list \
            ...
            CONFIG.Use_RSTA_Pin {false} \
            CONFIG.Use_RSTB_Pin {true} \
            CONFIG.Write_Depth_A [dict get ${AIT::project::aitConfig} "spawnout_queue_len"] \
            CONFIG.Write_Width_A {64} \
            CONFIG.Write_Width_B {32} \
            CONFIG.use_bram_block {Stand_Alone} \
        ] ${spawnOutQueue}
    ...
    connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/cmdout_in] [get_bd_intf_pins ${hwrInStreamHier}/cmdout_in]
    connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/cmdin_out] [get_bd_intf_pins axis_cmdin_TID/S_AXIS]
    connect_bd_intf_net [get_bd_intf_pins ${hwrOutStreamHier}/cmdin_out] [get_bd_intf_pins axis_cmdin_TID/M_AXIS]

    if {[dict get ${AIT::project::aitConfig} "task_creation"]} {
        connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/spawn_in] [get_bd_intf_pins ${hwrInStreamHier}/spawn_in]
        connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/spawn_out] [get_bd_intf_pins axis_spawn_TID/S_AXIS]
        connect_bd_intf_net [get_bd_intf_pins ${hwrOutStreamHier}/spawn_out] [get_bd_intf_pins axis_spawn_TID/M_AXIS]
        connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/taskwait_in] [get_bd_intf_pins ${hwrInStreamHier}/taskwait_in]
        connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/taskwait_out] [get_bd_intf_pins axis_taskwait_TID/S_AXIS]
        connect_bd_intf_net [get_bd_intf_pins ${hwrOutStreamHier}/taskwait_out] [get_bd_intf_pins axis_taskwait_TID/M_AXIS]
    }
    if {[dict get ${AIT::project::aitConfig} "lock_hwruntime"]} {
        connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/lock_in] [get_bd_intf_pins ${hwrInStreamHier}/lock_in]
        connect_bd_intf_net [get_bd_intf_pins ${hwruntimeIP}/lock_out] [get_bd_intf_pins axis_lock_TID/S_AXIS]
    ...
    connect_bd_intf_net -intf_net Picos_OmpSs_Manager_cmdInQueue [get_bd_intf_pins ${hwruntimeIP}/cmdin_queue] [get_bd_intf_pins cmdInQueue/BRAM_PORTA]
    connect_bd_intf_net -intf_net Picos_OmpSs_Manager_cmdOutQueue [get_bd_intf_pins ${hwruntimeIP}/cmdout_queue] [get_bd_intf_pins cmdOutQueue/BRAM_PORTA]
    ...
    if {![dict get ${AIT::project::aitConfig} "disable_spawn_queues"]} {
        connect_bd_intf_net -intf_net Picos_OmpSs_Manager_spawnInQueue [get_bd_intf_pins ${hwruntimeIP}/spawnin_queue] [get_bd_intf_pins spawnInQueue/BRAM_PORTA]
        connect_bd_intf_net -intf_net Picos_OmpSs_Manager_spawnOutQueue [get_bd_intf_pins ${hwruntimeIP}/spawnout_queue] [get_bd_intf_pins spawnOutQueue/BRAM_PORTA]
    ...
    if {[dict get ${AIT::project::aitConfig} "enable_pom_axilite"]} {
        connect_bd_intf_net -intf_net GP_Inter_pom_axilite [get_bd_intf_pins GP_Inter/M0${pom_axilite_m}_AXI] [get_bd_intf_pins ${hwruntimeIP}/axilite]
    }

    # Create port connections
    connect_bd_net [get_bd_pins clk] [get_bd_pins GP_Inter/ACLK] [get_bd_pins GP_Inter/M00_ACLK] [get_bd_pins GP_Inter/M01_ACLK] [get_bd_pins GP_Inter/S00_ACLK] [get_bd_pins ${hwruntimeIP}/clk] [get_bd_pins cmdInQueue_BRAM_Ctrl/s_axi_aclk] [get_bd_pins cmdOutQueue_BRAM_Ctrl/s_axi_aclk] [get_bd_pins ${hwrInStreamHier}/clk] [get_bd_pins ${hwrOutStreamHier}/clk] [get_bd_pins axis_cmdin_TID/aclk]
    ...
    connect_bd_net [get_bd_pins managed_rstn] [get_bd_pins ${hwruntimeIP}/rstn] [get_bd_pins axis_cmdin_TID/aresetn]
    ...
    if {[dict get ${AIT::project::aitConfig} "hwruntime_interconnect"] eq "centralized"} {
