...
if {[dict get ${AIT::vars::aitConfig} "task_creation"]} {
    ...
    AIT::AXIS::create_inStream_Inter_tree Hardware_Runtime/hwr_inStream/inS_ext_Inter 3 [dict get ${AIT::vars::aitConfig} "num_acc_creators"] $pi_clk $pi_inter_rstn $pi_peri_rstn
    # spawn_out + taskwait_out + outS_common_Inter_M00 (cmdin_out + lock_out if available)
    set max_level_ext [AIT::AXIS::create_outStream_Inter_tree Hardware_Runtime/hwr_outStream/outS_ext_Inter 3 [dict get ${AIT::vars::aitConfig} "num_acc_creators"] $po_clk $po_inter_rstn $po_peri_rstn]
...
set config_list [list CONFIG.M00_AXIS_BASETDEST {0x00000000} CONFIG.M00_AXIS_HIGHTDEST [format 0x%08X [expr {[dict get ${AIT::vars::aitConfig} "num_acc_creators"] - 1}]]]
set prev_inter_i 0
for {set i 0} {$i < [dict get ${AIT::vars::aitConfig} "num_instances"]} {incr i} {
    ...
    if {$i < [dict get ${AIT::vars::aitConfig} "num_acc_creators"]} {
    ...
    } elseif {[dict get ${AIT::vars::aitConfig} "num_acc_creators"] == 0} {
        ...
        set inter_i [expr {int(($i+1-[dict get ${AIT::vars::aitConfig} "num_acc_creators"])/16)}]
        set intf_i [format %02u [expr {($i + 1 - [dict get ${AIT::vars::aitConfig} "num_acc_creators"])%16}]]
        connect_bd_intf_net $inStream_intf [get_bd_intf_pins Hardware_Runtime/hwr_inStream/inS_common_Inter_lvl0_${inter_i}/S${intf_i}_AXIS]
        connect_bd_intf_net [get_bd_intf_pins Hardware_Runtime/hwr_outStream/outS_common_Inter_lvl0_${inter_i}/M${intf_i}_AXIS] $outStream_intf
