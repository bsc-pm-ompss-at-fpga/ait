...
namespace eval board {
    ...
    proc add_power_monitor {} {
        AIT::utils::error_msg "Board [dict get ${AIT::project::board} "name"] has no support for power monitoring"
    }

    # Placeholder for thermal monitor feature
    proc add_thermal_monitor {} {
        AIT::utils::error_msg "Board [dict get ${AIT::project::board} "name"] has no support for thermal monitoring"
    }

    # Maps board memory to address map
    proc configure_address_map {} {
        set boardMemType [dict get ${AIT::project::board} "memory" "type"]
        set boardMemBaseAddr [dict get ${AIT::project::board} "memory" "base_addr"]
        ...
        if {([dict get ${AIT::project::board} "arch" "device"] eq "zynq")
            ...
            set boardMemSize [dict get ${AIT::project::board} "memory" "size"]
        ...
        } elseif {[dict get ${AIT::project::board} "arch" "device"] eq "alveo"} {
            set boardMemBankSize [dict get ${AIT::project::board} "memory" "bank_size"]
            set boardMemNumBanks [dict get ${AIT::project::board} "memory" "num_banks"]
            ...
            if {[dict get ${AIT::project::aitConfig} "ompif"]} {
                ...
                assign_bd_address [get_bd_addr_segs ${AIT::templates::ethSubsys::container}/eth_100G_controller_0/s_axi/reg0] -range 16384 -offset 0x100000
                assign_bd_address [get_bd_addr_segs ${AIT::templates::ethSubsys::container}/eth_100G_rx_wrapper_0/s_axi/reg0] -range 16384 -offset 0x104000
                assign_bd_address [get_bd_addr_segs ${AIT::templates::OMPIF::messageSenderIP}/cntrl/reg0] -range 16384 -offset 0x108000
                assign_bd_address [get_bd_addr_segs ${AIT::templates::OMPIF::messageReceiverIP}/cntrl/Reg] -range 16384 -offset 0x114000
                assign_bd_address [get_bd_addr_segs jtag_gpio/S_AXI/Reg] -range 4096 -offset 0x10C000
                assign_bd_address -target_address_space ${AIT::templates::OMPIF::messageReceiverIP}/bufwr [get_bd_addr_segs axi_inter_msg_recv_bufwr/axiu_dwidth_downsize_0/slv/reg0]
                assign_bd_address -target_address_space ${AIT::templates::OMPIF::messageReceiverIP}/memcpy [get_bd_addr_segs axi_inter_msg_recv_memcpy/axiu_dwidth_downsize_0/slv/reg0]
                assign_bd_address -target_address_space ${AIT::templates::OMPIF::messageSenderIP}/moMEM [get_bd_addr_segs axi_inter_msg_send/axiu_dwidth_downsize_0/slv/reg0]
            }
        }
    }
    ...
    proc enable_mem_intf {intfDict} {
        if {([dict get ${AIT::project::board} "memory" "type"] eq "hbm") && ([dict get ${intfDict} "role"] eq "slave")} {
        ...
        } elseif {[dict get ${AIT::project::board} "arch" "device"] eq "zynq"} {
        ...
        } elseif {[dict get ${AIT::project::board} "arch" "device"] eq "zynqmp"} {
            set intfNum [dict get ${intfDict} "num"]
    ...
    proc generate_wrapper {} {
        set_property target_language [dict get ${AIT::project::aitConfig} "target_language"] [current_project]
    ...
    proc static_logic_register_slices {} {
        AIT::utils::warning_msg "Board [dict get ${AIT::project::board} "name"] has no support for static logic register slices"
    }
}
