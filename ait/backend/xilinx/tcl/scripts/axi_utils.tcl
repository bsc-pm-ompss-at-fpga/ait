...
namespace eval AXI {
    ...
    proc add_addrInterleaver {intfPin intfName {bdInstance /}} {
        ...
        if {([get_property TYPE ${intfPinIP}] ne "hier") || ([string match "xilinx.com:ip:axi_interconnect:*" [get_property VLNV ${intfPinIP}]])} {
            ...
            foreach addrBus ${addrBusList} {
                set numMemBanks [dict get ${AIT::project::board} "memory" "num_banks"]
                ...
                set_property -dict [list \
                    CONFIG.BANK_SIZE [dict get ${AIT::project::board} "memory" "bank_size"] \
                    CONFIG.NUM_BANKS ${numMemBanks} \
                    CONFIG.STRIDE "0x[format "%x" [dict get ${AIT::project::aitConfig} "memory_interleaving_stride"]]" \
                    CONFIG.BASE_ADDR [dict get ${AIT::project::board} "memory" "base_addr"] \
                ] ${intlvIP}
        ...
        # If interface has no address bus, look for it in a lower boundary
            ...
            set innerIntlvPinDict [add_addrInterleaver ${innerIntfPin} ${intfName} [get_bd_cells -of_objects ${intfPin}]]
    ...
    proc add_reg_slice {intfPin {masterSLR ""} {slaveSLR ""} {numPipelineStages ""} {regSliceName ""} {bdInstance .}} {
        ...
        lassign [split [dict get ${AIT::project::aitConfig} "regslice_pipeline_stages"] ':'] defaultNumStagesMaster defaultNumStagesMiddle defaultNumStagesSlave
        ...
        AIT::resets::connect_reset [get_bd_pins ${axiRegSliceIP}/aresetn] [AIT::resets::get_synchronous_rst_pin ${intfClkPin}]
    ...
    proc connect_intf {srcIntfPin dstIntf {clk ""} {rst ""}} {
        ...
        set_property -quiet CONFIG.STRATEGY [expr {([dict get ${AIT::project::aitConfig} "interconnect_opt"] eq "area") ? {1} : {2}}] ${dstIntfIP}

        if {(${dstIntfMode} eq "S") && [dict get ${AIT::project::aitConfig} "interconnect_priorities"]} {
        ...
        if {[dict get ${AIT::project::aitConfig} "interconnect_regslices"]} {
            set_property -dict [list \
        ...
        set dstInfPinRst [AIT::resets::get_associated_rst_pin ${dstIntfPinClk}]
        ...
        set srcIntfPinRst [AIT::resets::get_associated_rst_pin ${srcIntfPinClk}]
        ...
        if {${srcIntfPinRst} ne ""} {
            AIT::resets::connect_reset ${srcIntfPinRst} ${rst}
        }

        AIT::clocks::connect_clock ${dstIntfPinClk} [expr {(${clk} eq "") ? [AIT::clocks::get_associated_clk_pin ${srcIntfPin}] : ${clk}}]
        AIT::resets::connect_reset ${dstInfPinRst} [expr {(${rst} eq "") ? [AIT::resets::get_associated_rst_pin ${srcIntfPinClk}] : ${rst}}]
    ...
    proc connect_to_mem_intf {srcIntfPin {dst ""} {clk ""} {rst ""}} {
        ...
        if {${dst} ne ""} {
            set dstIntfIdx [lsearch -exact ${AIT::project::memIntfsList} [lsearch -regexp -inline -index 3 [lsearch -all -inline -index 1 ${AIT::project::memIntfsList} ${role}] ${dst}]]
            if {${dstIntfIdx} == -1} {
                AIT::utils::error_msg "Cannot connect ${srcIntfPin} to ${role} interface ${dst}. It does not exist"
            }
        } else {
            set dstIntfIdx [lsearch -index 1 ${AIT::project::memIntfsList} ${role}]
        }

        # Retrieve destination interface dictionary from the global list and remove it
        set dstIntf [lindex ${AIT::project::memIntfsList} ${dstIntfIdx}]
        set ::AIT::project::memIntfsList [lreplace ${AIT::project::memIntfsList} ${dstIntfIdx} ${dstIntfIdx}]

        # Connect source interface to destination interface
        set dstIntf [connect_intf ${srcIntfPin} ${dstIntf} ${clk} ${rst}]

        # Return updated interface dictionary to the global list and sort it by occupation
        lappend AIT::project::memIntfsList ${dstIntf}
        set ::AIT::project::memIntfsList [lsort -command AIT::utils::comp_dict -increasing ${AIT::project::memIntfsList}]
    ...
    proc create_mem_intf {intfDict} {
        dict with intfDict {
            ...
            AIT::resets::connect_reset [get_bd_pins ${intfIP}/ARESETN]
            ...
            foreach intfPinName ${pinBlock} {
                ...
                if {${parentDict} ne ""} {
                    if {${role} eq "master"} {
                        connect_intf [get_bd_intf_pins ${intfIP}/S${intfPinNum}_AXI] ${parentDict}
                    } else {
                        connect_intf [get_bd_intf_pins ${intfIP}/M${intfPinNum}_AXI] ${parentDict}
                    }
                } else {
                    ...
                    if {${intfPinMode} eq "Master"} {
                        ...
                        AIT::resets::connect_reset [AIT::resets::get_associated_rst_pin [AIT::clocks::get_associated_clk_pin ${intfSlavePin}]] [AIT::resets::get_synchronous_rst_pin [AIT::clocks::get_associated_clk_pin ${intfPin}]]
                        ...
                        AIT::resets::connect_reset [AIT::resets::get_associated_rst_pin [AIT::clocks::get_associated_clk_pin ${intfMasterPin}]] [AIT::resets::get_synchronous_rst_pin [AIT::clocks::get_associated_clk_pin ${intfPin}]]
                    }
                }
                incr intfPinNum
            }
    ...
    proc create_nested_interconnect {intfDict num} {
        ...
        set parentIntfIdx [lsearch -index 5 ${AIT::project::memIntfsList} [dict get ${intfDict} "IPName"]]
        set ::AIT::project::memIntfsList [lreplace ${AIT::project::memIntfsList} ${parentIntfIdx} ${parentIntfIdx}]
        for {set i 0} {${i} < ${num}} {incr i} {
            ...
            lappend AIT::project::memIntfsList ${newIntfDict}
