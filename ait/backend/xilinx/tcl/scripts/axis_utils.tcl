...
namespace eval AXIS {
    ...
    proc add_accID {intfPin accID {bdInstance .}} {
        ...
        set accIDWidth [expr {max(int(ceil(log([dict get ${AIT::project::aitConfig} "num_instances"])/log(2))), 1)}]
    ...
    proc add_newtask_spawner {intfPin inStreamPin outStreamPin {imp False} {bdInstance .}} {
        ...
        AIT::clocks::connect_clock [get_bd_pins ${tidDemuxIP}/clk] ${intfClkPin}
        ...
        set_property -dict [list \
            ...
            CONFIG.MAX_OWNS_PER_TASK [dict get ${AIT::project::aitConfig} "max_deps_per_task"] \
        ] ${newtaskSpawnerIP}

        if {${imp}} {
            connect_bd_net [get_bd_pins ${newtaskSpawnerIP}/ompif_rank] [get_bd_pins ${AIT::project::OMPIF}/ompif_rank]
        }
    ...
    proc add_stream_adapter {intfPin {accID 0x0} {bdInstance .}} {
        ...
        if {[get_property DIR ${intfPin}] eq "O"} {
            ...
            set_property -dict [list \
                CONFIG.TID_WIDTH [expr {max(int(ceil(log([dict get ${AIT::project::aitConfig} "num_instances"])/log(2))), 1)}] \
                CONFIG.ACCID ${accID} \
            ] ${streamAdapterIP}
