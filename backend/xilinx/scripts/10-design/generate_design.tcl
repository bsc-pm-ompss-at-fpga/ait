...
proc configureAddressMap {address_map} {
    ...
    upvar #0 arch_device arch_device interleaving_stride interleaving_stride
    ...
    if {($arch_device eq "zynq") || ($arch_device eq "zynqmp")} {
        ...
        if {$interleaving_stride ne "None"} {
            assign_bd_address [get_bd_addr_segs -regexp .*_addrInterleaver/S_AXI/reg0]
            include_bd_addr_seg [get_bd_addr_segs -excluded -regexp .*_addrInterleaver_reg0]
        }
    }
}
    ...
    for {set j 0} {$j < $accNumInstances} {incr j} {
        ...
        foreach acc_AXIPort $list_acc_AXIPorts {
            ...
            if {$interleaving_stride ne "None"} {
                ...
                connect_bd_intf_net [get_bd_intf_pins $acc_AXIPort] [get_bd_intf_pins $addrInterleaver/S_AXI]
                connect_bd_net [get_bd_pins $addrInterleaver/AXI_aclk] [get_bd_pins ${accName}_$j/aclk]
                connect_bd_net [get_bd_pins $addrInterleaver/AXI_aresetn] [get_bd_pins ${accName}_$j/managed_aresetn]
                set_property CONFIG.ADDR_WIDTH [get_property CONFIG.ADDR_WIDTH [get_bd_intf_pins $acc_AXIPort]] [get_bd_cells ${accName}_$j/${port_name}_addrInterleaver]
                set_property CONFIG.DATA_WIDTH [get_property CONFIG.DATA_WIDTH [get_bd_intf_pins $acc_AXIPort]] [get_bd_cells ${accName}_$j/${port_name}_addrInterleaver]
                set_property CONFIG.FREQ_HZ [get_property CONFIG.FREQ_HZ [get_bd_intf_pins $acc_AXIPort]] [get_bd_intf_pins -regexp ${accName}_$j/${port_name}_addrInterleaver/._AXI]
                set hier_AXIPort $addrInterleaver/M_AXI
            }
