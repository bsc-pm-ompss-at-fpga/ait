...
def synthesize_accelerator(acc):
    ...
    acc_tcl_script = '# Script automatically generated by the Accelerator Integration Tool. Edit at your own risk.\n' \
                     ...
                     + 'set_top ' + acc.full_name + '_wrapper\n' \
    ...
    acc_tcl_script_file = open(project_backend_path + '/HLS/' + acc.short_name + '/HLS_' + acc.short_name + '.tcl', 'w')
...
def run_HLS_step(project_args):
    ...
    if len(accs) > num_accs:
        msg.info('Synthesizing ' + str(len(accs) - num_accs) + ' additional support IP' + ('s' if len(accs) - num_accs > 1 else ''))
